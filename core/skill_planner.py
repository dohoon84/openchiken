...
## 중요 — 외부 에이전트 위임 우선 규칙:
- 사용자가 "oc_ext_xxx", "oc_ext_xxx 에이전트를 통해서", "다른 에이전트에게 물어봐" 등
  특정 에이전트 ID 또는 외부 에이전트 위임을 명시한 경우, 반드시 **relay_discover + relay_invoke** 스킬을 선택하세요.
  로컬에서 직접 처리 가능하더라도 relay 경로를 우선합니다.

## 중요 — 로컬 스킬 없을 때 외부 에이전트 탐색:
- 필요한 기능이 **설치된 스킬** 목록에도 없고 **허브 스킬** 목록에도 없다면,
  **relay_discover + relay_invoke** 스킬을 선택하세요.
  Relay에 연결된 외부 에이전트 중 해당 기능을 가진 에이전트를 찾아 위임합니다.
- relay_discover와 relay_invoke가 설치된 스킬 목록에 있으면, 위 상황에서 반드시 포함시키세요.
