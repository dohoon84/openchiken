...
## 외부 에이전트 호출 도구 (사용자 명시 요청 전용)

OpenChiken Relay 서버를 통해 다른 외부 에이전트에게 작업을 위임합니다.

⚠️ **이 도구는 일반 작업(검색, 조회, 분석 등)에 사용하지 마세요.**
로컬 도구로 처리 가능한 작업은 항상 로컬 도구를 먼저 사용하세요.
...
- 사용자가 "oc_ext_xxx 에이전트한테 물어봐줘" 등 **명시적으로** 외부 에이전트를 지정할 때
- 사용자가 "다른 에이전트에게 위임해줘"라고 **직접 요청**할 때

### 사용 흐름

1. discover_agents로 사용 가능한 외부 에이전트와 agentId 확인
